            ...
            <div>
                ...
                <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        <tr>
                            <td>
                                <form method="post" action="/pengajar/tugas/{{ $tugas->kode_tugas }}">
                                    @method('put')
                                    @csrf
                                    <div class="row">
                                            <!-- Nama Tugas -->
                                            <div class="col-lg-6 mt-2">
                                                ...
                                                <x-text-input id="nama_tugas" class="block mt-1 w-full" type="text" name="nama_tugas" :value="old('nama_tugas', $tugas->nama_tugas)" required />
                                            ...
                                            <div class="col-lg-6 mt-2">
                                                ...
                                                <select id="kode_kelas" name="kode_kelas" class="block appearance-none w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                                    ...
                                                    <option value="{{ $kelompok->kode_kelas }}" {{ $tugas->kode_kelas === $kelompok->kode_kelas ? 'selected' : '' }}>{{ $kelompok->nama_kelas }}</option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <div class="col-lg-6 mt-2">
                                                ...
                                                <select id="kode_materi" name="kode_materi" class="block appearance-none w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                                    @foreach ($materis as $materi)
                                                        <option value="{{ $materi->kode_materi }}" {{ $tugas->kode_materi === $materi->kode_materi ? 'selected' : '' }}>{{ $materi->nama_materi }}</option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <div class="col-lg-6 mt-2">
                                                ...
                                                <x-date-input id="tanggal_pengumpulan" class="block mt-1 w-full" type="date"
                                                    name="tanggal_pengumpulan" :value="old('tanggal_pengumpulan', $tugas->tanggal_pengumpulan)" required autocomplete="off" />
                                                <x-input-error :messages="$errors->get('tanggal_pengumpulan')" class="mt-2" />
                                            </div>
                                            <!-- Deskripsi -->
                                            <div class="col-lg-6 mt-2">
                                                <x-input-label for="deskripsi" :value="__('Deskripsi')" />
                                                <textarea id="deskripsi" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" name="deskripsi" required>{{ old('deskripsi',$tugas->deskripsi) }}</textarea>
                                                <x-input-error :messages="$errors->get('deskripsi')" class="mt-2" />
                                            </div>
                                            <!-- Keterangan -->
                                            <div class="col-lg-6 mt-2">
                                                <x-input-label for="keterangan" :value="__('Keterangan')" />
                                                <textarea id="keterangan" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" name="keterangan" required>{{ old('keterangan',$tugas->keterangan) }}</textarea>
