            ...
            <div>
                ...
                <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        <tr>
                            <td>
                                <form method="post" action="/admin/dashboard/siswa/{{ $siswa->kode_siswa }}">
                                    @method('patch')
                                    @csrf
                                    <div class="row">
                                        <!-- Kode User -->
                                        <div class="col-lg-6 mt-2">
                                            ...
                                            <select id="kode_user" name="kode_user" class="block appearance-none w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                                @foreach ($users as $user)
                                                    <option value="{{ $user->kode_user }}" {{ $siswa->kode_user === $user->kode_user ? 'selected' : '' }}>{{ $user->nama_lengkap }}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="col-lg-6 mt-2">
                                            ...
                                            <select id="kode_pendaftaran" name="kode_pendaftaran" class="block appearance-none w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                                @foreach ($pendaftarans as $pendaftaran)
                                                    <option value="{{ $pendaftaran->kode_pendaftaran }}" {{ $siswa->kode_pendaftaran === $pendaftaran->kode_pendaftaran ? 'selected' : '' }}>{{ $pendaftaran->kode_pendaftaran }} - {{ $pendaftaran->user->nama_lengkap }}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="col-lg-6 mt-4">
                                            ...
                                            <x-date-input id="tanggal_masuk" class="block mt-1 w-full" type="date"
                                                name="tanggal_masuk" :value="old('tanggal_masuk', $siswa->tanggal_masuk)" autocomplete="off" />
                                        ...
                                        <div class="col-lg-6 mt-4">
                                            ...
                                            <x-date-input id="tanggal_lulus" class="block mt-1 w-full" type="date"
                                                name="tanggal_lulus" :value="old('tanggal_lulus'. $siswa->tanggal_lulus)" autocomplete="off" />
